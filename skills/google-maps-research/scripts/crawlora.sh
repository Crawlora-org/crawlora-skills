...
case "$method" in
  GET|POST) ;;
  *)
    echo "only GET and POST are supported by the google-maps-research skill" >&2
    exit 2
    ;;
esac
...
case "$path" in
  ""|*[?#%]*|*..*|*//* )
    echo "invalid path for the google-maps-research skill" >&2
    exit 2
    ;;
esac
...
if [ "$route_allowed" = false ]; then
  echo "path is not in the google-maps-research skill catalog" >&2
  exit 2
fi
...
if [ "$route_method_allowed" = false ]; then
  echo "method is not allowed for this google-maps-research route" >&2
  exit 2
fi
