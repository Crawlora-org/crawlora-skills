...
while [ $# -gt 0 ]; do
  case "$1" in
    -X|-d) echo "only GET are supported by the linkedin-research skill" >&2; exit 2 ;;
    *) args+=("$1"); shift ;;
  esac
done
...
case "$method" in
  GET) ;;
  *)
    echo "only GET are supported by the linkedin-research skill" >&2
    exit 2
    ;;
esac
...
case "$path" in
  ""|*[?#%]*|*..*|*//* )
    echo "invalid path for the linkedin-research skill" >&2
    exit 2
    ;;
esac
...
if [ "$route_allowed" = false ]; then
  echo "path is not in the linkedin-research skill catalog" >&2
  exit 2
fi
