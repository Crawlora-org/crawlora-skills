...
while [ $# -gt 0 ]; do
  case "$1" in
    -X|-d) echo "only GET are supported by the hiring-demand-analysis skill" >&2; exit 2 ;;
    *) args+=("$1"); shift ;;
  esac
done
...
case "$method" in
  GET) ;;
  *)
    echo "only GET are supported by the hiring-demand-analysis skill" >&2
    exit 2
    ;;
esac
...
case "$path" in
  ""|*[?#%]*|*..*|*//* )
    echo "invalid path for the hiring-demand-analysis skill" >&2
    exit 2
    ;;
esac
...
if [ "$route_allowed" = false ]; then
  echo "path is not in the hiring-demand-analysis skill catalog" >&2
  exit 2
fi
