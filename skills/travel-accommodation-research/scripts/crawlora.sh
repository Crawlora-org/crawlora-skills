...
case "$method" in
  GET|POST) ;;
  *)
    echo "only GET and POST are supported by the travel-accommodation-research skill" >&2
    exit 2
    ;;
esac
...
case "$path" in
  ""|*[?#%]*|*..*|*//* )
    echo "invalid path for the travel-accommodation-research skill" >&2
    exit 2
    ;;
esac
...
if [ "$route_allowed" = false ]; then
  echo "path is not in the travel-accommodation-research skill catalog" >&2
  exit 2
fi
