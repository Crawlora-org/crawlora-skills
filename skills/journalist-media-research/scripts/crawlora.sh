...
case "$method" in
  GET|POST) ;;
  *)
    echo "only GET and POST are supported by the journalist-media-research skill" >&2
    exit 2
    ;;
esac
...
case "$path" in
  ""|*[?#%]*|*..*|*//* )
    echo "invalid path for the journalist-media-research skill" >&2
    exit 2
    ;;
esac
...
route_method_regexes=(
  '^GET:/bing/news$'
  '^GET:/bing/search$'
  '^GET:/datasets/journalists/facets$'
  '^GET:/datasets/journalists/items/[^/]+/[^/]+$'
  '^GET:/datasets/journalists/search$'
  '^POST:/web/scrape$'
)
...
if [ "$route_method_allowed" = false ]; then
  echo "method is not allowed for this journalist-media-research route" >&2
  exit 2
fi
