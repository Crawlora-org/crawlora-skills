...
route_method_regexes=(
  ...
  '^GET:/bing/search$'
  '^GET:/datasets/apple-podcasts-shows/facets$'
  '^GET:/datasets/apple-podcasts-shows/items/[^/]+$'
  '^GET:/datasets/apple-podcasts-shows/search$'
...
if [ "$route_method_allowed" = false ]; then
  echo "method is not allowed for this podcast-guest-research route" >&2
  exit 2
fi
