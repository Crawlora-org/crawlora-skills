...
case "$method" in
  ...
  *)
    echo "only DELETE and GET and PATCH and POST are supported by the website-monitoring skill" >&2
    exit 2
    ;;
esac
...
case "$path" in
  ""|*[?#%]*|*..*|*//* )
    echo "invalid path for the website-monitoring skill" >&2
    exit 2
    ;;
esac
...
if [ "$route_allowed" = false ]; then
  echo "path is not in the website-monitoring skill catalog" >&2
  exit 2
fi
...
if [ "$route_method_allowed" = false ]; then
  echo "method is not allowed for this website-monitoring route" >&2
  exit 2
fi
