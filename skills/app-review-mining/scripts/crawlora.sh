...
while [ $# -gt 0 ]; do
  case "$1" in
    -X|-d) echo "only GET are supported by the app-review-mining skill" >&2; exit 2 ;;
    *) args+=("$1"); shift ;;
  esac
done
...
case "$method" in
  GET) ;;
  *)
    echo "only GET are supported by the app-review-mining skill" >&2
    exit 2
    ;;
esac
...
case "$path" in
  ""|*[?#%]*|*..*|*//* )
    echo "invalid path for the app-review-mining skill" >&2
    exit 2
    ;;
esac
