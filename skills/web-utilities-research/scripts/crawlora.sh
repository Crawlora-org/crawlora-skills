...
case "$method" in
  GET|POST) ;;
  *)
    echo "only GET and POST are supported by the web-utilities-research skill" >&2
    exit 2
    ;;
esac
...
case "$path" in
  ""|*[?#%]*|*..*|*//* )
    echo "invalid path for the web-utilities-research skill" >&2
    exit 2
    ;;
esac
...
if [ "$route_method_allowed" = false ]; then
  echo "method is not allowed for this web-utilities-research route" >&2
  exit 2
fi
