...
case "$method" in
  GET|POST) ;;
  *)
    echo "only GET and POST are supported by the startup-acquisition-research skill" >&2
    exit 2
    ;;
esac
...
case "$path" in
  ""|*[?#%]*|*..*|*//* )
    echo "invalid path for the startup-acquisition-research skill" >&2
    exit 2
    ;;
esac
...
route_method_regexes=(
  '^GET:/bing/search$'
  '^GET:/datasets/trustmrr/facets$'
  '^GET:/datasets/trustmrr/history/[^/]+$'
  '^GET:/datasets/trustmrr/items/[^/]+$'
  '^GET:/datasets/trustmrr/search$'
...
if [ "$route_method_allowed" = false ]; then
  echo "method is not allowed for this startup-acquisition-research route" >&2
  exit 2
fi
