...
case "$method" in
  GET|POST) ;;
  *)
    echo "only GET and POST are supported by the local-competitive-landscape skill" >&2
    exit 2
    ;;
esac
...
case "$path" in
  ""|*[?#%]*|*..*|*//* )
    echo "invalid path for the local-competitive-landscape skill" >&2
    exit 2
    ;;
esac
...
if [ "$route_allowed" = false ]; then
  echo "path is not in the local-competitive-landscape skill catalog" >&2
  exit 2
fi
...
route_method_regexes=(
  ...
  '^GET:/datasets/google-map-businesses/items/[^/]+$'
...
if [ "$route_method_allowed" = false ]; then
  echo "method is not allowed for this local-competitive-landscape route" >&2
  exit 2
fi
