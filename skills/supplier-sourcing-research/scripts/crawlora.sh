...
case "$method" in
  GET|POST) ;;
  *)
    echo "only GET and POST are supported by the supplier-sourcing-research skill" >&2
    exit 2
    ;;
esac
...
case "$path" in
  ""|*[?#%]*|*..*|*//* )
    echo "invalid path for the supplier-sourcing-research skill" >&2
    exit 2
    ;;
esac
...
if [ "$route_allowed" = false ]; then
  echo "path is not in the supplier-sourcing-research skill catalog" >&2
  exit 2
fi
...
if [ "$route_method_allowed" = false ]; then
  echo "method is not allowed for this supplier-sourcing-research route" >&2
  exit 2
fi
