...
case "$method" in
  GET|POST) ;;
  *)
    echo "only GET and POST are supported by the techstack-prospecting skill" >&2
    exit 2
    ;;
esac
...
case "$path" in
  ""|*[?#%]*|*..*|*//* )
    echo "invalid path for the techstack-prospecting skill" >&2
    exit 2
    ;;
esac
case "$path" in
  ...
  /web/techstack) ;;
  *)
    echo "path is not in the techstack-prospecting skill catalog" >&2
    exit 2
    ;;
esac
